...
class HelmRouteParser {
  ...
  NavigationState _parseComplexPath(String path, Uri uri) {
    ...
    final contextStack = <List<Page<Object?>>>[<Page<Object?>>[]];
...
//   final _TrieNode _trieRoot = _TrieNode();
//   final LinkedHashMap<String, List<String>> _matchCache = LinkedHashMap();
//   final LinkedHashMap<String, List<String>> _segmentCache = LinkedHashMap();
//   final LinkedHashMap<String, NavigationState> _parseCache = LinkedHashMap();
...
//     if (_parseCache.length >= _maxParseResults) {
//       final keysToRemove = _parseCache.keys.take(_maxParseResults ~/ 4);
//       for (final key in keysToRemove) {
//         _parseCache.remove(key);
//       }
//     }
...
//     return [homeRoute.page(queryParams: uri.queryParameters)];
//   }
...
//       if (isDive) {
//         if (contextStack.last.isEmpty) return const [];
//         final parentPage = contextStack.last.last;
//         final parentArgs = parentPage.meta;
//         if (parentArgs == null) return const [];
...
//         contextStack.add(newChildren);
//       } else {
//         if (contextStack.length <= 1) return const [];
//         contextStack.removeLast();
//       }
//       remainingPath = remainingPath.substring(delimiterIndex + 2);
//     }
